...
public class ScheunenScraper implements Scraper {
    ...
    try {
      ...
      for (Element headerOrEvent : doc.select("h3.head.monat, div.va")) {
        if (headerOrEvent.tagName().equals("h3")) {
          ...
          // Parse description
          String description =
              headerOrEvent.select("a.term.reihe_link").text()
                  + "\u003cbr\u003e"
                  + headerOrEvent.select("a.va_location").text();
          ...
          if (titleElement != null) {
            ...
            java.util.regex.Matcher mEin =
                java.util.regex.Pattern.compile(
                        "Einlass\\s*(ab\\s*)?(\\d{1,2}:\\d{2})\\s*Uhr",
                        java.util.regex.Pattern.CASE_INSENSITIVE)
                    .matcher(allText);
            if (mEin.find()) {
              try {
                einlassTime = LocalTime.parse(mEin.group(2), TIME_FMT);
              } catch (Exception ignored) {
              }
            }
            java.util.regex.Matcher mBeg =
                java.util.regex.Pattern.compile("(\\d{1,2}:\\d{2})\\s*Uhr").matcher(allText);
            if (beginnTime == null && mBeg.find()) {
              try {
                beginnTime = LocalTime.parse(mBeg.group(1), TIME_FMT);
              } catch (Exception ignored) {
              }
            }
            ...
            java.util.regex.Matcher mFree =
                java.util.regex.Pattern.compile("(?i)eintritt( ist)? frei").matcher(allText);
            if (mFree.find()) {
              priceBits.add("Eintritt frei");
            }
            // AK: ... and VVK: ... limited tokens (up to 25 chars of typical price chars)
            java.util.regex.Matcher mAk =
                java.util.regex.Pattern.compile("(?i)\\bAK:?\\s*([\\p{L}\\p{N}€.,/\\-– ]{1,25})")
                    .matcher(rightText);
            while (mAk.find()) {
              String val = mAk.group(1).trim();
              if (!val.isEmpty()) priceBits.add("AK: " + val);
            }
            java.util.regex.Matcher mVvk =
                java.util.regex.Pattern.compile("(?i)\\bVVK:?\\s*([\\p{L}\\p{N}€.,/\\-– ]{1,25})")
                    .matcher(rightText);
            while (mVvk.find()) {
            ...
            java.util.regex.Matcher mHeaderLoc =
                java.util.regex.Pattern.compile(
            ...
            if (mHeaderLoc.find()) {
              ...
              java.util.regex.Matcher mLoc =
                  java.util.regex.Pattern.compile("scheune\\s+([A-Za-zÄÖÜäöüß]+)").matcher(allText);
              if (mLoc.find()) {
            ...
            Element thumbnailElement =
                detailDoc.selectFirst("figure a[rel='galerie[]'] img, figure img");
          ...
          newEvent.setSourceUrl(
              detailPageUrl != null && !detailPageUrl.isBlank() ? detailPageUrl : BASE_URL);
  ...
  public static Month getMonthFromGerman(String germanMonth) {
    switch (germanMonth.toLowerCase(java.util.Locale.GERMAN)) {
      ...
      default:
        throw new IllegalArgumentException("Invalid month: " + germanMonth);
    }
  }
}
