...
public interface EventRepository extends JpaRepository<Event, Long> {
  ...
  @Query(
      value =
          "SELECT e FROM Event e WHERE e.date >= :date AND (LOWER(e.title) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(e.description) LIKE LOWER(CONCAT('%', :q, '%'))) ORDER BY e.date ASC",
      countQuery =
          "SELECT COUNT(e) FROM Event e WHERE e.date >= :date AND (LOWER(e.title) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(e.description) LIKE LOWER(CONCAT('%', :q, '%')))")
  Page<Event> searchUpcoming(
      @Param("q") String q, @Param("date") LocalDate date, Pageable pageable);
  ...
  @Query(
      ...
      countQuery =
          "SELECT COUNT(e) FROM Event e "
              ...
              + "AND (:loc IS NULL OR LOWER(e.location) = LOWER(:loc)) "
              + "AND ((:q IS NULL OR :q = '') OR LOWER(e.title) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(e.description) LIKE LOWER(CONCAT('%', :q, '%')))")
  Page<Event> searchUpcomingWithFilters(
      @Param("q") String q,
