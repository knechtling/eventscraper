...
public class AdminController {
  ...
  public String scraperHealth(Model model) {
      ...
      if (last != null) {
        boolean running =
            last.getFinishedAt() == null
                && last.getStartedAt() != null
                && java.time.Duration.between(last.getStartedAt(), now).toMinutes() < 15;
        if (running) {
          status = "RUNNING";
        } else if (last.getErrors() > 0) {
          status = "ERROR";
        } else if (last.getFinishedAt() != null
            && java.time.Duration.between(last.getFinishedAt(), now).toHours() <= 24) {
          status = "OK";
        } else {
          status = "STALE";
        }
    ...
    model.addAttribute(
        "runs", scrapeRunRepository.findAllByOrderByStartedAtDesc(PageRequest.of(0, 50)));
    return "scrapers";
  }
  ...
  public String runScrapers(
      @RequestParam(value = "name", required = false) String name, RedirectAttributes ra) {
    try {
  ...
  public String runScrapersAlias(@RequestParam(value = "name", required = false) String name) {
    ...
    return "redirect:/admin/scraper-health/run?name="
