...
class EventControllerFilterTest {
  ...
  @MockBean EventRepository eventRepository; // needed for /event/details mapping in controller
  ...
  void passes_filters_to_service_and_renders_model() throws Exception {
    ...
    Mockito.when(
            eventService.searchWithFilters(
    ...
    Mockito.when(eventService.getUniqueLocationsFromUpcoming())
        .thenReturn(List.of("Chemiefabrik", "Hanse 3", "Scheune Dresden"));
    ...
    mockMvc
        ...
        .andExpect(
            model()
                .attributeExists(
                    "eventsPage", "events", "locations", "search", "start", "end", "loc"))
